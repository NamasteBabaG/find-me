...
describe('contextual release judge',()=>{
 it('requires two passing independent reviews and records both costs, images and wire requests',async()=>{
  ...
  const r=await new OpenAiPatchJudge('test').judge(input());expect(r.verdict).toBe('ok');expect(r.version).toBe(BOARD_JUDGE_VERSION);expect(r.reviews).toHaveLength(2);expect(r.attempts).toHaveLength(2);expect(r.costCents).toBeCloseTo(2);expect(r.imageHashes).toHaveLength(3);
  const bodies=fetch.mock.calls.map(c=>JSON.parse(c[1].body));expect(bodies[0].max_tokens).toBe(320);expect(bodies[1].max_completion_tokens).toBe(4096);expect(bodies[1].reasoning_effort).toBe('medium');expect(bodies[0].messages[0].content.filter((x:any)=>x.type==='image_url')).toHaveLength(3);
