...
export const BOARD_JUDGE_VERSION = "board-quality-v3-consensus";
...
export const BOARD_CHECKS = ["identity", "faceIntegrity", "bodyPlacement", "style"] as const;
export type BoardChecks = Record<(typeof BOARD_CHECKS)[number], "pass" | "fail" | "uncertain">;

export function boardJudgePrompt(childName: string): string {
  return [
    ...
    'Return JSON only: {"checks":{"identity":"pass|fail|uncertain","faceIntegrity":"pass|fail|uncertain","bodyPlacement":"pass|fail|uncertain","style":"pass|fail|uncertain"},"reason":"brief concrete visible evidence; identify the actual occluding/supporting object or its absence"}.',
  ].join(" ");
}
