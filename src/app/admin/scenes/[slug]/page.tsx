...
export default async function AdminScenePreviewPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!scene) notFound();
  const demo = buildDemoConfig(slug, "מאיה").scenes[0]!;
  return (
    <div className="fm-stack fm-stack--3">
      ...
      <h1>
        {scene.name} <span className="fm-badge fm-badge--outline">v{scene.version}</span> <span className="fm-badge fm-badge--outline">{scene.artStatus}</span>
      </h1>
      <p className="fm-muted">{scene.tagline}</p>
      ...
      <div className="fm-card fm-stack fm-stack--1">
        ...
        <ul className="fm-small">
          {scene.ambient.map((a) => (
            <li key={a.id}>
              {a.glyph} {a.label} — ({a.x}, {a.y}) · {a.animation} · {a.reaction}
            </li>
          ))}
        </ul>
        <strong>חגיגה</strong>
        <span className="fm-small">
          {scene.celebration.kind} · {scene.celebration.completeText} · פריט: {scene.collectible.icon} {scene.collectible.name}
        </span>
      </div>
    </div>
  );
