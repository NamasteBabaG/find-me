...
import { createCanonicalIdentityReuse, CANONICAL_IDENTITY_REUSE_CONFIRMATION } from "@/services/generation/local-patch-identity-reuse";
...
async function copyIdentity(data: FormData) {
  ...
  const result = await createCanonicalIdentityReuse(getContainer(), {
    ...
    requestId: String(data.get("requestId") ?? ""), displayName: String(data.get("displayName") ?? ""), confirmation: String(data.get("confirmation") ?? ""),
...
export default async function IdentityReusePage({ searchParams }: { searchParams: Promise<{ sourceGameId?: string; identityAssetId?: string }> }) {
  ...
  return <section className="fm-stack fm-stack--2" dir="rtl">
    ...
    <p>המשחק החדש: 9 לוחות, 45 מחבואים, גרסה 8 עם בדיקות זהות ותפר. תקרת יצירה כוללת: $4. יצירת המחבואים תתחיל רק אחרי אישור התשלום המדומה במסך הבא.</p>
    <form action={copyIdentity} className="fm-stack fm-stack--2">
      ...
      <label><input type="checkbox" name="confirmation" value={CANONICAL_IDENTITY_REUSE_CONFIRMATION} required /> אני מאשר יצירת משחק QA חדש עם האיור הזה בדיוק, ללא שינוי המשחק המקורי.</label>
