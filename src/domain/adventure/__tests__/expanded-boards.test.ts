...
import {ADVENTURE_EXPANDED_BOARDS,EXPANDED_PATCH_BOARDS} from '../../../../content/adventures/expanded-boards';
...
const fixture=()=>{
 const boards=EXPANDED_PATCH_BOARDS,catalog=ADVENTURE_EXPANDED_BOARDS;
...
describe('explicit nine-board expansion',()=>{
 ...
 it('keeps all item-card pixels outside every personal return patch',()=>{
  for(const b of ADVENTURE_EXPANDED_BOARDS.boards){if(b.status!=='ready')throw Error('not ready');expect([b.art.width,b.art.height]).toEqual([3840,2160]);expect(b.discoveries.map(d=>d.rarity)).toEqual(['common','common','common','rare','rare','epic']);
   const p=EXPANDED_PATCH_BOARDS.find(p=>p.board===b.boardSlug)!;
 ...
 it('does not mutate the family-tested three-board catalog',()=>{
  ...
  expect(EXPANDED_PATCH_BOARDS[1]!.hides[1]!.placement!.standingHeightPx).toBe(440);
 });
