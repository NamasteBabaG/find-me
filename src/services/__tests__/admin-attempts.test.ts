...
describe("attempts for the admin page", () => {
  it("names the stage, the pictures and the judgement of each attempt", () => {
    ...
    expect(a[0]).toMatchObject({ n: 1, renderAssetId: "ast_r1", matteAssetIds: ["ast_m1", "ast_m2"], patchAssetId: "ast_p1", compositeAssetId: "ast_c1", hiddenFraction: 0.55 });
    // Each wire image keeps its own role and hash, so a missing picture cannot shift the others.
    expect(a[0]!.judgeImages).toEqual([
      { role: "board", sha256: "aa", assetId: "ast_j1" },
      { role: "patch", sha256: "bb", assetId: "ast_j2" },
      { role: "sheet", sha256: "cc", assetId: null },
    ]);
