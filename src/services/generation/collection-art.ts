...
export const COLLECTION_ART_ORIGIN = "https://qa.findmeworlds.com";

/** Read the exact published pixels, never a model URL or a request-supplied host.
 * Local verification reads disk. Deployed QA reads its authenticated static CDN:
...
export async function readCollectionArt(art: string, expectedHash: string, root = process.cwd(),
  ...
  if (!remote) {
    ...
    if (process.env.VERCEL !== "1" || !config.enabled) throw Error("COLLECTION_ART: packaged pixels missing outside deployed QA");
    if (cache.has(entry.sha256)) return cache.get(entry.sha256)!;
    remote = { fetch: globalThis.fetch, cookie: `${qaCookieName(config)}=${await createQaSession(config)}` };
  }
  const response = await remote.fetch(`${COLLECTION_ART_ORIGIN}/${art.slice(7)}`, {
    method: "GET", redirect: "error", cache: "no-store", signal: AbortSignal.timeout(20_000),
    headers: { cookie: remote.cookie, accept: "image/webp" },
  });
