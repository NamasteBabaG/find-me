...
export function PassportSharing({ childId }: { childId: string }) {
  ...
  async function send(operation: string) {
    ...
    try {
      ...
      if ("active" in data) setActive(Boolean(data.active));
  ...
  return <section className="passport-sharing" aria-label={copy.title}>
    ...
    {open ? <div className="passport-sharing__panel">
      <h2>{copy.title}</h2><p>{copy.warning}</p><p>{copy.lifetime}</p>
      {adult ? <><p>{copy.adult}</p><button className="fm-btn" disabled={busy} onClick={() => void send("reauth")}>{copy.verify}</button></> : <>
        {!preview ? <button className="fm-btn fm-btn--secondary" disabled={busy} onClick={() => void send("preview")}>{copy.preview}</button> : <>
          ...
          {!active ? <button className="fm-btn" disabled={busy || !confirmed} onClick={() => void send("enable")}>{copy.enable}</button> : null}
        </>}
        {active ? <div className="passport-sharing__links"><p>{copy.active}</p>{url ? <><button className="fm-btn" onClick={() => { void navigator.clipboard.writeText(url).then(() => setMessage(copy.copied)).catch(() => setMessage(copy.copyFailed)); }}>{copy.copy}</button><input aria-label={copy.link} value={url} readOnly onFocus={e => e.currentTarget.select()} /></> : <p>{copy.copyOnce}</p>}<button className="fm-btn fm-btn--ghost" disabled={busy} onClick={() => setConfirmation("revoke")}>{copy.revoke}</button><button className="fm-btn fm-btn--ghost" disabled={busy || !confirmed || !preview} onClick={() => setConfirmation("rotate")}>{copy.rotate}</button></div> : null}
      </>}
