 ...
 *
 *   npx tsx scripts/generate-world.ts city [--quality=medium] [--size=1536x1024] [--refs=beach,jungle,space] [--dry]
 *
 * Output:
...
async function main() {
  ...
  form.append("n", "1");
  console.log(`calling OpenAI images/edits (${flag("model", "gpt-image-2")}, ${flag("quality", "medium")}, ${size}, ${refs.length} style refs)…`);
  ...
  await sharp(png).webp({ quality: 85 }).toFile(path.join(dir, "base.webp"));
  await sharp(png).resize({ width: 480 }).webp({ quality: 80 }).toFile(path.join(dir, "thumb.webp"));
  await sharp({ create: { width: W, height: H, channels: 4, background: { r: 0, g: 0, b: 0, alpha: 0 } } }).webp({ quality: 80, alphaQuality: 100 }).toFile(path.join(dir, "foreground.webp"));
  ...
  scene.artStatus = "draft";
  writeFileSync(scenePath, JSON.stringify(scene, null, 2) + "\n");
  console.log(`✓ ${slug}: ${W}×${H} → public/scenes/${slug}/ (base, thumb, empty foreground); scene.json updated (still inactive until slots are placed)`);
}
