import {assertDensityApproval} from './lib/adventure-density-approval';
...
import {LocalPatchBoardSchema,LocalPatchHideSchema,maskForHide,assertPlaceable} from '../src/domain/scene/local-patch-hides';
...
const WORLD='adventure-bar-density-20260914-v1',CAP=3_000_000;
const hash=(b:Buffer|string)=>createHash('sha256').update(b).digest('hex');
async function main(){
 const [slug,phase='--dry-run',target]=process.argv.slice(2);

 if(!slug||!['paris','marrakech','tokyo','greatwall','sydney','antarctica'].includes(slug)||!['--dry-run','--render','--repair'].includes(phase))throw Error('Choose a frozen expansion board and explicit phase');
 if(phase==='--repair'&&!target)throw Error('A repair requires exactly one hide id');
 const raw=JSON.parse(readFileSync(`content/adventures/density-v3/${slug}.json`,'utf8'));
 const board=LocalPatchBoardSchema.parse(raw.patchBoard),plan=ReadyAdventureBoardSchema.parse(raw.plan);
 assertDensityApproval(slug,board,plan);
 ...
 const identity=readFileSync(`${prior}/identity-normalized.png`),dir=path.resolve('storage/adventure-bar-density-20260914');mkdirSync(dir,{recursive:true});
 ...
 try{
  ...
  const extra=repair?.attempt===4&&target==='adventure-marrakech-density-v3-3'&&repair.extraAttemptReason==='Final local repair of a visually rejected neighbor-cutting result; within the existing $3 ceiling, no automatic continuation';
  if(repair&&(repair.inputsSha256!==hash(serialized)||(![2,3].includes(repair.attempt)&&!extra)||repair.hideId!==target))throw Error('Invalid explicit repair plan');
  const hides=target?board.hides.filter(h=>h.id===target):board.hides;
  if(!hides.length)throw Error('No matching hide');
  for(const originalHide of hides){
   const hide=LocalPatchHideSchema.parse({...originalHide,...(repair?.maskOverride?{mask:repair.maskOverride}:{}),...(repair?.placementOverride?{placement:{...originalHide.placement,...repair.placementOverride}}:{})});
   const m=maskForHide(hide);if(Math.min(m.left,m.top,512-m.left-m.width,768-m.top-m.height)<SEAM_LIMITS.bandPx)throw Error(`Unsafe authored seam margin before purchase: ${hide.id}`);
   ...
   writeFileSync(`${prefix}.json`,JSON.stringify({accepted:result.accepted,inputsSha256:hash(serialized),sha256:result.shippingPng?hash(result.shippingPng):null,refusedBecause:result.refusedBecause,renderFault:result.renderFault,seam:result.seam,compositionPermission:result.compositionPermission,stoppedReason:result.stoppedReason,costUnknown:result.costUnknown,renderCents:result.renderCents,replayed:result.replayed,visualReview:'pending',...(repair?{repairSha256:hash(JSON.stringify(repair)),effectiveHide:hide}:{})},null,2));
