 ...
 *
 *   npx tsx scripts/slot-patch.ts import beach sandcastle A path/to/edited.png [--threshold=28] [--feather=6] [--out=public/demo/patches]
...
async function exportCrop(slug: string, targetId: string, variantArg?: string) {
  ...
  const rx = Math.round(c.childPx * 0.55);
  const ry = Math.round(c.childPx * 0.8);
  const mask = Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${c.rect.w}" height="${c.rect.h}"><rect width="100%" height="100%" fill="#000"/><ellipse cx="${cx}" cy="${cy}" rx="${rx}" ry="${ry}" fill="#fff"/></svg>`);
  await sharp(mask).png().toFile(path.join(WORK, `${c.name}.mask.png`));
...
async function importPatch(slug: string, targetId: string, variantArg: string | undefined, editedPath: string) {
  ...
  // 1. Changed pixels (max channel difference above the threshold).
