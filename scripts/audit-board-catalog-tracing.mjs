...
const routes = manifests.map(manifest => {
  ...
  return { manifest: rel(manifest), tracedFiles: paths.size, uncompressedBytes: bytes,
    ...
    missingFiles, privateFiles, staleCatalogFiles, publicCdnFiles: [...paths].map(rel).filter(file => /^public\/(scenes|worlds)\//.test(file)).length };
});
...
if (routes.some(route => route.publicCdnFiles)) problems.push("public CDN scene art duplicated inside server function");
