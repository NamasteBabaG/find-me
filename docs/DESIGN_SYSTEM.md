...
## The album (pilot, `src/game/components/Album.tsx`)

A discovery is a tap of its own on the board (`SceneViewport`: exact, then the same 64px reach as a child, never a guess between two; the child wins a shared tap). Its reaction never stops the search: `twinkle`, a bubble with the name, a small burst and a 2.6 s toast "a new card in the album"; a second tap says "already in the album" with no toast. Cards and postcards are CSS windows onto the board's existing pixels (`AlbumCrop`, `background-size: 100/w%`), the postcard adds the child's own patch of the first find; no image is generated. The bag shows, per place, the postcard or "N more hiding spots for the postcard", the discovery cards (name + story when found, "not found yet" + hint when not), and one honest line on where the album is kept (`game.album.savedAccount / saving / offline / guest / unreadable`). Three finds open the next place and the postcard needs every hiding spot: said at the choice moment (`.scene__advance-note`) and in the album, never as a fixed line in the search HUD. Tokens and the 8px grid throughout; at ≤720px the album is one column and the bag's star meter moves under the name.
