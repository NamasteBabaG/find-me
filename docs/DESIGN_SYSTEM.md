...
## The passport (2026-09-17)

- **It is a book, and it belongs to this product.** The first build gave the passport its own palette — teal covers, mint grounds, sage type, about forty raw hex values — and the result was a well-made object from a different company. Paper is `--paper`, ink is `--ink`, the cover is `--night` with `--gold` foil, and "found" is `--leaf`: the same green that ticks a discovery in the game's tray, so one mark means one thing everywhere.
- **Character comes from material, not from a private palette.** Dark cloth board with a debossed foil keyline, a page block visible edge-on, paper with a faint tooth, and a real crease down the fold — dark where the paper bends, bright just off it. Two soft gutter washes alone read as a seam; a book has a line.
- **The stamp sinks into the page.** `mix-blend-mode: multiply` lets the paper grain show through the ink, which is the whole difference between a stamp and a badge. It lands the way a hand brings one down: big and askew, pressed past its resting angle, then settled.
- **A keepsake not found yet is a waiting place, never a failure.** Up to six dashed boxes each captioned "not found yet" put six small notices of what the child had not done down one page. It is a shallow well pressed into the paper with a single faint mark instead; the words stay in the markup for a screen reader.
- **The book yields before the page does.** A 510px floor on a 640px-tall phone left about 130px for a toolbar, an index strip and a status line, so the page scrolled ~98px and the index fell below the fold — on the screen size a child is most likely to read on. On a short phone the floor relaxes and the contents step down with it; every viewport now sits at zero vertical and zero horizontal overflow.
- **The crease is desktop-only.** On a phone there is one page, so there is no fold: drawn there it runs straight down the middle of the picture.
